...
//******/
+ (JWTBuilder *)encodePayloadStr:(NSString *)payloadStr {
    return [JWTBuilder encodePayloadStr:payloadStr];
}
...
//******/
@property (copy, nonatomic, readwrite) NSString *jwtPayloadStr;
...
//*********/
@property (strong, nonatomic, readwrite) JWTBuilder *(^payloadStr)(NSString *payloadStr);
...
- (NSString *)jwtPayloadStr {
    return _jwtClaimsSet ? [JWTClaimsSetSerializer stringWithClaimsSet:_jwtClaimsSet] : _jwtPayloadStr;
}
...
- (instancetype)payloadStr:(NSString *)payloadStr {
    self.jwtPayloadStr = payloadStr;
    return self;
}
...
//*******/
+ (JWTBuilder *)encodePayloadStr:(NSString *)payloadStr {
    return [[JWTBuilder alloc] init].payloadStr(payloadStr);
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //********/
        self.payloadStr = ^(NSString *payloadStr) {
            return [weakSelf payloadStr:payloadStr];
        };
...
- (NSString *)encodeHelper1
{
    ...
    NSString *payloadSegment;
    
        payloadSegment = [self encodeSegmentStr:self.jwtPayloadStr withError:nil];
...
- (NSString *)encodeSegment:(id)theSegment withError:(NSError **)error
{
    ...
    NSString *cleanJsonString = [[[encodedSegment stringByReplacingOccurrencesOfString:@"+" withString:@"-"] stringByReplacingOccurrencesOfString:@"/" withString:@"_"] stringByReplacingOccurrencesOfString:@"=" withString:@""];
...
- (NSString *)encodeSegmentStr:(NSString*)theSegment withError:(NSError **)error
{
...
//    NSString *encodedSegment = nil;
//    
//    NSData *data123 = [theSegment dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:false];
//    
//    if (theSegment) {
//        encodedSegmentData = [NSJSONSerialization JSONObjectWithData:data123 options:NSJSONReadingAllowFragments error:nil];
//        
//        if (encodedSegmentData) {
//            encodedSegment = [JWTBase64Coder base64UrlEncodedStringWithData:encodedSegmentData];
//        }
...
//    
    ...
    NSData *nsdata = [theSegment dataUsingEncoding:NSUTF8StringEncoding];
    
    // Get NSString from NSData object in Base64
    NSString *encodedSegment = [nsdata base64EncodedStringWithOptions:0];
    
    //NSString *encodedSegment = [JWTBase64Coder base64UrlEncodedStringWithData:encodedSegmentData];
