...
- (void)renderCluster:(id<GMUCluster>)cluster animated:(BOOL)animated {
    ...
    if ([self shouldRenderAsCluster:cluster atZoom:zoom]) {
        ...
        GMSMarker *marker = [self markerWithPosition:cluster.position
                                         ...
                                         clusterIcon:icon
                                            animated:animated];
        MarkerView *clusterMarker = nil;
        if (marker.iconView == nil) {
            clusterMarker = [self createDefaultMarkerView];
        }
        else{
            if ([marker.iconView isKindOfClass:[MarkerView class]]) {
                NSArray *arrClusterItems = cluster.items;
                if (arrClusterItems.count > 0) {
                    if ([arrClusterItems.firstObject isKindOfClass:[POIItem class]]) {
                        MarkerView *clusterView = (MarkerView *)marker.iconView;
                        clusterMarker = clusterView;
                    }
                }
            }
            else{
        ...
        if (clusterMarker != nil) {
            ...
            if (arrClusterItems.count > 0) {
                if ([arrClusterItems.firstObject isKindOfClass:[POIItem class]]) {
                    POIItem *clusterItem = arrClusterItems.firstObject;
                    Equipment *equipmentObj = clusterItem.equipmentObj;
                    [clusterMarker loadMarkerImageFromImageUrl:equipmentObj.iconImage];
                    clusterMarker.imageCount.image = icon;
                    marker.iconView = clusterMarker;
                }
            }
        }
        ...
        for (id<GMUClusterItem> item in cluster.items) {
            ...
            MarkerView *markerView = nil;
            if (marker.iconView == nil) {
                markerView = [self createDefaultMarkerView];
            }else{
                if ([marker.iconView isKindOfClass:[MarkerView class]]) {
                    MarkerView *clusterView = (MarkerView *)marker.iconView;
                    markerView = clusterView;
                }
                else{
                    marker.iconView = nil;
                    markerView = [self createDefaultMarkerView];
            ...
            if (markerView != nil) {
                if ([item isKindOfClass:[POIItem class]]) {
                    POIItem *clusterItem = item;
                    Equipment *equipmentObj = clusterItem.equipmentObj;
                    [markerView loadMarkerImageFromImageUrl:equipmentObj.iconImage];
                    markerView.imageCount.image = nil;
...
/*-(ClusterMarkerView *)createClusterMarkerView{
    ClusterMarkerView *clusterView = [[UINib nibWithNibName:@"ClusterMarkerView" bundle:nil]instantiateWithOwner:nil options:nil][0];
    if ([[UIScreen mainScreen]bounds].size.width > 320) {
        CGRect originalFrame = clusterView.frame;
        originalFrame.size.width = 50;
        originalFrame.size.height = 70;
        clusterView.frame = originalFrame;
    }
    return clusterView;
}*/
-(MarkerView *)createDefaultMarkerView{
    MarkerView *markerView = [[UINib nibWithNibName:@"MarkerView" bundle:nil]instantiateWithOwner:nil options:nil][0];
    if ([[UIScreen mainScreen]bounds].size.width > 320) {
        CGRect originalFrame = markerView.frame;
        originalFrame.size.width = 48;
        originalFrame.size.height = 53;
        markerView.frame = originalFrame;
    }
    return markerView;
}
...
- (GMSMarker *)markerWithPosition:(CLLocationCoordinate2D)position from:(CLLocationCoordinate2D)from userData:(id)userData clusterIcon:(UIImage *)clusterIcon animated:(BOOL)animated {
