...
class PastOrdersViewController: ProviderBaseViewController,UITableViewDataSource,UITableViewDelegate,UITextViewDelegate {

    @IBOutlet var tblPastOrders : UITableView?
    var arrPastOrders : NSArray?
    var selectedOrder : Order?
    var cancelReasonAlert : UIView?
    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view.
        tblPastOrders?.tableFooterView = UIView()
        tblPastOrders?.estimatedRowHeight = 400
        self.recordScreenView("PastOrdersViewController", FSP_Past_Orders_Screen)
        self.registerFirebaseEvents(PV_FSP_Past_Orders_Tab, "", "", "", parameters: nil)
    }
    ...
    func relodData(){
        if arrPastOrders?.count == 0{
            ...
            self.tblPastOrders?.tableFooterView = UIView()
        }
        self.tblPastOrders?.reloadData()
    }
    //MARK: UITableview delegate and datasource methods
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return arrPastOrders?.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let order = arrPastOrders?.object(at: indexPath.row) as? Order
    ...
    @IBAction func proposeChangesButtonClick(_ sender: UIButton){
        let buttonPosition = sender.convert(CGPoint(x:0,y:0), to: self.tblPastOrders)
        let indexPath = self.tblPastOrders!.indexPathForRow(at: buttonPosition)
        if indexPath != nil {
            if let order = arrPastOrders?.object(at: indexPath!.row) as? Order{
    ...
    @IBAction func rejectOrderButtonClick(_ sender: UIButton){
        ...
        if indexPath != nil {
            if let order = arrPastOrders?.object(at: indexPath!.row) as? Order{
                ...
                cancelReasonAlert = CustomAlert.orderCancelAndRejectPopup(self, title: Order_Cancel_Alert_Title as NSString, frame: UIScreen.main.bounds, okButtonTitle: "OK") as? UIView
