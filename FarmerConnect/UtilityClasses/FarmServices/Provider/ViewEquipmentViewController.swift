//
//  ViewEquipmentViewController.swift
//  FarmerConnect
//
...
class ViewEquipmentViewController: ProviderBaseViewController,UICollectionViewDelegate,UICollectionViewDataSource,UITableViewDelegate,UITableViewDataSource,UIPopoverPresentationControllerDelegate,UICollectionViewDelegateFlowLayout {
 ...
 //   var arrProviderTitles = ["Maker","Model","Performance","With driver","Pick and drop","Year of manufacture","Service area distance(in kms)","Minimum service(in hours)","Description :"]
   var arrProviderTitles = [NSLocalizedString("maker", comment: ""),NSLocalizedString("model", comment: ""),NSLocalizedString("performance", comment: ""),NSLocalizedString("with_driver", comment: ""),NSLocalizedString("pick_drop", comment: ""),NSLocalizedString("year_manufacture", comment: ""),NSLocalizedString("service_area_distance_kms", comment: ""),NSLocalizedString("minimum_service_hours", comment: ""),NSLocalizedString("description", comment: "")]
    ...
    override func viewDidLoad() {
        ...
        navMoreOptionsButton.addTarget(self, action: #selector(ViewEquipmentViewController.moreOptionsButtonClick(_:)), for: UIControlEvents.touchUpInside)
        ...
        shareButton.addTarget(self, action: #selector(ViewEquipmentViewController.shareButtonClick(_:)), for: UIControlEvents.touchUpInside)
        ...
        self.recordScreenView("ViewEquipmentViewController", View_Equipment)
        if isFromRequester {
                                    ...
                                    if  self.viewEquipment != nil{
                                        if Validations.isNullString(( self.viewEquipment?.equipmentId) ?? "") == false{
                                                    let userObj = Constatnts.getUserObject()
                                                                 ...
                                                                 let bookEquipmentController = self.storyboard?.instantiateViewController(withIdentifier: "BookNowViewController") as? BookNowViewController
       ...
       @IBAction func shareButtonClick( _ sender: UIButton)
    {
        if self.viewEquipment != nil {
            let userObj = Constatnts.getUserObject()
            let urlPath = String(format: "%@=%@&%@=%@&%@=%@", Module,Equipment_View_Details,Equipment_Id,self.viewEquipment?.equipmentId ?? "",Equipment_Owner_Id,self.viewEquipment?.ownerId ?? "")
    ...
    @IBAction func moreOptionsButtonClick( _ sender: UIButton)
    {
        ...
        if self.selectedEquipment?.status != "Disabled" {
            ...
            btnDisable.addTarget(self, action: #selector(ViewEquipmentViewController.disableEquipmentButtonClick(_:)), for: UIControlEvents.touchUpInside)
        }
        else{
            btnDisable.setTitle(NSLocalizedString("enable", comment: ""), for: .normal)
            btnDisable.addTarget(self, action: #selector(ViewEquipmentViewController.enableEquipmentButtonClick(_:)), for: UIControlEvents.touchUpInside)
        }
        ...
        btnDelete.addTarget(self, action: #selector(ViewEquipmentViewController.deleteEquipmentButtonClick(_:)), for: UIControlEvents.touchUpInside)
