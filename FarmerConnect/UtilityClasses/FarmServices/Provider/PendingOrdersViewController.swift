...
class PendingOrdersViewController: ProviderBaseViewController,UITableViewDelegate,UITableViewDataSource,UITextViewDelegate {

    @IBOutlet var tblPendingOrders : UITableView?
    var arrPendingOrders : NSArray?
    var selectedOrder : Order?
    var cancelReasonAlert : UIView?
    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view.
        tblPendingOrders?.tableFooterView = UIView()
        tblPendingOrders?.estimatedRowHeight = 400
        self.recordScreenView("PendingOrdersViewController", FSP_Pending_Orders_Screen)
        self.registerFirebaseEvents(PV_FSP_Pending_Orders_Tab, "", "", "", parameters: nil)
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return arrPendingOrders?.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let order = arrPendingOrders?.object(at: indexPath.row) as? Order
        ...
        if order?.status == "Pending" {
            lblStatus?.textColor = App_Theme_Orange_Color
        }
    ...
    @IBAction func proposeChangesButtonClick(_ sender: UIButton){
        let buttonPosition = sender.convert(CGPoint(x:0,y:0), to: self.tblPendingOrders)
        let indexPath = self.tblPendingOrders!.indexPathForRow(at: buttonPosition)
        if indexPath != nil {
            if let order = arrPendingOrders?.object(at: indexPath!.row) as? Order{
    ...
    @IBAction func rejectOrderButtonClick(_ sender: UIButton){
        ...
        if indexPath != nil {
            if let order = arrPendingOrders?.object(at: indexPath!.row) as? Order{
                ...
                cancelReasonAlert = CustomAlert.orderCancelAndRejectPopup(self, title: Order_Cancel_Alert_Title as NSString, frame: UIScreen.main.bounds, okButtonTitle: NSLocalizedString("ok", comment: "") as NSString) as? UIView
    ...
    @IBAction func confirmOrderButtonClick(_ sender: UIButton){
        ...
        if indexPath != nil {
            if let order = arrPendingOrders?.object(at: indexPath!.row) as? Order{
                let userObj = Constatnts.getUserObject()
                let fireBaseParams = [MOBILE_NUMBER: userObj.mobileNumber!,USER_ID: userObj.customerId,EQUIPMENT_ID:order.equipmentId,ORDER_ID:order.equipmentTransactionId] as [String : Any]
            ...
            if let order = arrPendingOrders?.object(at: indexPath!.row) as? Order{
                if let mobilNumer = order.contactNo as String?{
                    ...
                    if let url = URL(string: callUrl), UIApplication.shared.canOpenURL(url) {
                        self.registerFirebaseEvents(FSP_Call_Requester, "", "", "", parameters: nil)
                        if #available(iOS 10, *) {
