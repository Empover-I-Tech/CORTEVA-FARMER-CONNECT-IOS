//
//  RHRDFieldSharingViewController.swift
//  FarmerConnect
//
//  Created by Empover on 13/09/21.
...
class RHRDFieldSharingViewController: BaseViewController,UIScrollViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate , UICollectionViewDelegate,UICollectionViewDataSource{
    ...
    var statementArray = NSMutableArray()
    var AllowCorteva = "No"
    @IBOutlet weak var lbl_allowCoretveMsg: UILabel!
    @IBOutlet weak var btn_allowCoretve: UIButton!
    ...
    override func viewDidLoad() {
        ...
        selectSharingStatement.text = NSLocalizedString("cep_Field_Sharing_Select", comment: "")
        ...
        lbl_allowCoretveMsg.text = NSLocalizedString("RHRD_AllowCoretve_msg", comment: "")
        sharingStatement.reloadData()
        shareOnMediaStack.isHidden = true
        
        let refreshButton = UIButton(type: .custom)
        refreshButton.frame = CGRect(x: (self.topView?.frame.size.width)! - 40, y: 10, width: 30, height: 30)
    ...
    @IBAction func allowCorevteButtonClick(_ sender : UIButton){
        
        if AllowCorteva == "No"{
            AllowCorteva = "Yes"
            btn_allowCoretve.setImage(UIImage(named: "check_selected_Blue"), for: .normal)
        }else{
            AllowCorteva = "No"
            btn_allowCoretve.setImage(UIImage(named: "unchecked_Blue"), for: .normal)
        }
    }
    ...
    @IBAction func previewAndShareAction(_ sender: Any) {
        if isimageSelected && isMessageSelected{
            ...
            if net?.isReachable == true{
                ...
                let dic : NSDictionary = [
                    ...
                    "selectedStatement": selectedStr,
                    "allowCorteva": AllowCorteva,
        ...
        else if isimageSelected && !isMessageSelected {
            self.view.makeToast(NSLocalizedString("cep_Field_Sharing_Select", comment: ""))
        }
    ...
    func uploadingWithMultiPartFormData( _ str : String,nmae: String){
        ...
        Alamofire.upload(multipartFormData: {(multipartFormData) in
            ...
            //            }
        }, usingThreshold: UInt64.init(), to: String(format :"%@%@",BASE_URL,GET_RHRD_FIELDSHARING), method: .post, headers: headers, encodingCompletion: {(encodeResult) in
            switch encodeResult {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        arrayFieldSharing = [ NSLocalizedString("RHRD_Field_ExceptionalCrop", comment: ""),
                              NSLocalizedString("RHRD_FieldStmt_More_flowering", comment: ""),
                              NSLocalizedString("RHRD_FieldStmt_Goodquality_chilli", comment: ""),
                              NSLocalizedString("RHRD_FieldStmt_BestClass_controll", comment: "")
        ]
        
        self.lblTitle?.text = NSLocalizedString("rhrd_uploadField_and_get_luckydraw", comment: "")
    ...
    @IBAction func whatsAppShare(_ sender: Any)
    {
        ...
        // let urlWhats = "https://api.whatsapp.com/send/?phone=919985789922&photos=\(img_upload.image)"
        if let urlString = urlWhats.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed) {
            // 3
            if let whatsappURL = NSURL(string: urlString) {
                // 4
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if arrSelected.contains(arrayFieldSharing.object(at: indexPath.row) as? String ?? ""){
            shareButton?.setImage(UIImage(named: "SelectRadioBlue"), for: .normal)
        }
...
extension RHRDFieldSharingViewController : UIDocumentInteractionControllerDelegate{
...
extension RHRDFieldSharingViewController: SharingDelegate {
