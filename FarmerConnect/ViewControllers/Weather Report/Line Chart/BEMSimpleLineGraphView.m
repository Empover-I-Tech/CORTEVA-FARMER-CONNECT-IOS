...
@interface BEMSimpleLineGraphView ()<UIScrollViewDelegate> {
    ...
    /// All of the X-Axis Labels
    NSMutableArray *circleDotsArray;
...
- (void)commonInit {
    ...
    circleDotsArray = [NSMutableArray array];
    ...
    _contentScrollView.backgroundColor = self.colorBackgroundXaxis;
    //_contentScrollView.pagingEnabled = YES;
    [self addSubview:_contentScrollView];
    
    _pageControl = [[UIPageControl alloc]init];
    _pageControl.frame = CGRectMake(0, self.frame.size.height - 40, self.frame.size.width, 35);
    [self addSubview:_pageControl];
    _pageControl.alpha = 0.0;
    [self bringSubviewToFront:_contentScrollView];
    self.backgroundColor = self.colorBackgroundXaxis;
...
- (void)prepareForInterfaceBuilder {
    ...
    numberOfPoints = 10;
    for (UILabel *subview in [self.contentScrollView subviews]) {
...
- (void)layoutNumberOfPoints {
    ...
    if ([self.dataSource respondsToSelector:@selector(numberOfPointsInLineGraph:)]) {
        ...
        CGFloat contentWidth = self.backgroundXAxis.frame.size.width + (self.contentScrollView.frame.size.width);
        self.contentScrollView.contentSize = CGSizeMake(contentWidth, _backgroundXAxis.frame.size.height);
    ...
    if (numberOfPoints == 0) {
       ...
       // NSLog(@"[BEMSimpleLineGraph] Data source contains only one data point. Add more data to the data source and then reload the graph.");
        ...
        circleDot.alpha = 1;
        [self.contentScrollView addSubview:circleDot];
        return;
...
- (void)layoutTouchReport {
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        //[self.contentScrollView.viewForBaselineLayout addSubview:self.panView];
...
- (void)drawDots {
    ...
    [circleDotsArray removeAllObjects];
    ...
    @autoreleasepool {
        for (int i = 0; i < numberOfPoints; i++) {
            ...
            if (self.positionYAxisRight) {
                ...
                positionOnXAxis = (((self.backgroundXAxis.frame.size.width) / (numberOfPoints - 1)) * i) + self.YAxisLabelXOffset ;
                //positionOnXAxis = (self.YAxisLabelXOffset + (self.graphSpacing * i)); /// (numberOfPoints - 1)) * i) + self.YAxisLabelXOffset;
            }
            ...
            if (dotValue != BEMNullGraphValue) {
                if (i == 0 || i == numberOfPoints - 1) {
                    positionOnXAxis = positionOnXAxis - self.sizePoint/2;
                }
                else{
                    positionOnXAxis = positionOnXAxis - self.sizePoint/2;
                }
                BEMCircle *circleDot = [[BEMCircle alloc] initWithFrame:CGRectMake(positionOnXAxis, positionOnYAxis - self.sizePoint/2, self.sizePoint, self.sizePoint)];
                ...
                circleDot.Pointcolor = self.colorPoint;
                //NSNumber *xAxisLabelCoordinate = [NSNumber numberWithFloat:positionOnXAxis ];
                //[xAxisLabelPoints addObject:xAxisLabelCoordinate];

                [self.backgroundXAxis addSubview:circleDot];
                ...
                [circleDotsArray addObject:circleDot];
                _pageControl.numberOfPages = circleDotsArray.count;
...
- (void)drawLine {
    ...
    CGRect lineFrame = [self drawableGraphArea];
    lineFrame.origin.x = 0;
    lineFrame.size.width = lineFrame.size.width + self.contentScrollView.frame.size.width ;
    BEMLine *line = [[BEMLine alloc] initWithFrame:lineFrame];
    ...
    [self.backgroundXAxis addSubview:line];
    [self.backgroundXAxis sendSubviewToBack:line];
    [self.contentScrollView sendSubviewToBack:self.backgroundXAxis];
    CGFloat contentWidth = self.backgroundXAxis.frame.size.width + (self.contentScrollView.frame.size.width);
    self.contentScrollView.contentSize = CGSizeMake(contentWidth, _backgroundXAxis.frame.size.height);
...
- (void)drawXAxis {
    ...
    [self.contentScrollView addSubview:self.backgroundXAxis];
    CGFloat contentWidth = self.backgroundXAxis.frame.size.width + (self.contentScrollView.frame.size.width);
    self.contentScrollView.contentSize = CGSizeMake(contentWidth, _backgroundXAxis.frame.size.height);
    ...
    if ([self.delegate respondsToSelector:@selector(incrementPositionsForXAxisOnLineGraph:)]) {
        ...
        for (NSNumber *increment in axisValues) {
            ...
            UILabel *labelXAxis = [self xAxisLabelWithText:xAxisLabelText atIndex:index previousXPosition:0];
        ...
        while (startingIndex < numberOfPoints) {
            ...
            UILabel *labelXAxis = [self xAxisLabelWithText:xAxisLabelText atIndex:startingIndex previousXPosition:0];
            [xAxisLabels addObject:labelXAxis];
        ...
        if (numberOfGaps >= (numberOfPoints - 1)) {
            ...
            CGFloat viewWidth = self.backgroundXAxis.frame.size.width; //- self.YAxisLabelXOffset;
            ...
            UILabel *firstLabel = [self xAxisLabelWithText:firstXLabel atIndex:0 previousXPosition:0];
            firstLabel.frame = CGRectMake(xAxisXPositionFirstOffset, self.contentScrollView.frame.size.height - 20, viewWidth/2, 20);
            
            firstLabel.textAlignment = NSTextAlignmentLeft;
            [self.contentScrollView addSubview:firstLabel];
            [xAxisValues addObject:firstXLabel];
            [xAxisLabels addObject:firstLabel];
            
            UILabel *lastLabel = [self xAxisLabelWithText:lastXLabel atIndex:numberOfPoints - 1 previousXPosition:0];
            ...
            [self.contentScrollView addSubview:lastLabel];
            ...
            @autoreleasepool {
                ...
                for (int i = 1; i <= (numberOfPoints/numberOfGaps); i++) {
                    ...
                    x_Position = (((self.backgroundXAxis.frame.size.width) / (numberOfPoints - 1)) * index) + self.YAxisLabelXOffset ;
                    NSString *xAxisLabelText = [self xAxisTextForIndex:index];
                    UILabel *labelXAxis = [self xAxisLabelWithText:xAxisLabelText atIndex:index previousXPosition:x_Position];
                    [xAxisLabels addObject:labelXAxis];
                    if (self.positionYAxisRight) {
                        NSNumber *xAxisLabelCoordinate = [NSNumber numberWithFloat:labelXAxis.center.x];
                        [xAxisLabelPoints addObject:xAxisLabelCoordinate];
                    } else {
                        NSNumber *xAxisLabelCoordinate = [NSNumber numberWithFloat:labelXAxis.frame.origin.x ];
                        [xAxisLabelPoints addObject:xAxisLabelCoordinate];
                    }
                    CGRect xLabelFrame = labelXAxis.frame;
                    xLabelFrame.origin.x = (xLabelFrame.origin.x - xLabelFrame.size.width/2);
                    labelXAxis.frame = xLabelFrame ;
...
- (UILabel *)xAxisLabelWithText:(NSString *)text atIndex:(NSInteger)index previousXPosition:(float)x_pos {
    ...
    CGRect lRect = [labelXAxis.text boundingRectWithSize:self.contentScrollView.viewForFirstBaselineLayout.frame.size options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:labelXAxis.font} context:nil];
    lRect.size.width = _graphSpacing;
    CGPoint center;
    ...
    if (index == 0) {
        horizontalTranslation = 0;//lRect.size.width/2;
    ...
    if (self.positionYAxisRight) {
        ...
        positionOnXAxis = (((self.backgroundXAxis.frame.size.width) / (numberOfPoints - 1)) * index) + self.YAxisLabelXOffset + horizontalTranslation;
        //positionOnXAxis =(((self.contentScrollView.frame.size.width - self.YAxisLabelXOffset) / (numberOfPoints - 1)) * index) + self.YAxisLabelXOffset + horizontalTranslation;
    }
    positionOnXAxis = positionOnXAxis;
    // Set the final center point of the x-axis labels
    if (self.positionYAxisRight) {
        center = CGPointMake(positionOnXAxis, self.contentScrollView.frame.size.height - lRect.size.height/2);
    } else {
        center = CGPointMake(positionOnXAxis, self.contentScrollView.frame.size.height - lRect.size.height/2);
    }
    ...
    rect.origin.x = x_pos;
    rect.origin.y = (self.contentScrollView.frame.size.height - lRect.size.height) - 2;
...
//  CGRectMake(xAxisXPositionFirstOffset, self.frame.size.height-20, viewWidth/2, 20);
    ...
    CGFloat viewWidth = numberOfPoints * self.graphSpacing - self.YAxisLabelXOffset; //self.frame.size.width - self.YAxisLabelXOffset;
    CGFloat adjustedHeight = self.contentScrollView.frame.size.height - xAxisHeight;
...
- (CGRect)drawableXAxisArea {
    NSInteger xAxisHeight = self.contentScrollView.frame.size.height;
    NSInteger xAxisWidth = [self drawableGraphArea].size.width + self.contentScrollView.frame.size.width;
    CGFloat xAxisXOrigin = self.positionYAxisRight ? 0 : self.contentScrollView.frame.size.width/2;//self.YAxisLabelXOffset;
    CGFloat xAxisYOrigin = self.contentScrollView.frame.size.height - xAxisHeight;
...
- (NSInteger)offsetForXAxisWithNumberOfGaps:(NSInteger)numberOfGaps {
    ...
    NSInteger rightGap = numberOfPoints - (numberOfGaps*(numberOfPoints * self.graphSpacing/numberOfGaps));
    NSInteger offset = 0;
...
- (void)reloadGraph {
    for (UIView *subviews in self.contentScrollView.subviews) {
...
- (void)handleGestureAction:(UIGestureRecognizer *)recognizer {
    ...
    if ([panGesture respondsToSelector:@selector(velocityInView:)]) {
        CGPoint velocity = [panGesture velocityInView:self.panView];
        if(velocity.x > 0)
        {
...
-(void)closestPointAnimationWithScrollGesture:(UIGestureRecognizer *)recognizer{
    CGPoint translation = [recognizer locationInView:self.contentScrollView.viewForLastBaselineLayout];
    if (!((translation.x + self.frame.origin.x) <= self.frame.origin.x) && !((translation.x + self.frame.origin.x) >= self.frame.origin.x + self.backgroundXAxis.frame.size.width)) { // To make sure the vertical line doesn't go beyond the frame of the graph.
        self.touchInputLine.frame = CGRectMake(translation.x - self.widthTouchInputLine/2, 0, self.widthTouchInputLine, self.frame.size.height);
        //NSLog(@"Came Inside");
    }
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    [self changePageNumber:scrollView];
    /*if ((scrollView.contentOffset.x + self.contentScrollView.frame.size.width - (self.frame.size.width/2 - self.YAxisLabelXOffset)) > 0 && (scrollView.contentOffset.x) < (scrollView.contentSize.width - (self.contentScrollView.frame.size.width + self.YAxisLabelXOffset))) {
        int pageNumber = round((scrollView.contentOffset.x - (self.contentScrollView.frame.size.width/2)) / _graphSpacing);
        _pageControl.currentPage = pageNumber;
        for (BEMCircle *circle in circleDotsArray) {
            circle.alpha = 0.0;
        }
        if (pageNumber < circleDotsArray.count) {
            BEMCircle *circle = [circleDotsArray objectAtIndex:pageNumber];
            circle.alpha = 1.0;
        }
    }*/
    //UIPanGestureRecognizer *scrollPanGesture = scrollView.panGestureRecognizer;
    //[self closestPointAnimationWithScrollGesture:scrollPanGesture];
}
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    [self changePageNumber:scrollView];
    /*if ((scrollView.contentOffset.x - (self.frame.size.width/2 - self.YAxisLabelXOffset)) > 0 && (scrollView.contentOffset.x) < (scrollView.contentSize.width - (self.contentScrollView.frame.size.width + self.YAxisLabelXOffset))) {
        int pageNumber = round((scrollView.contentOffset.x - (self.contentScrollView.frame.size.width/2)) / _graphSpacing);
        _pageControl.currentPage = pageNumber;
        for (BEMCircle *circle in circleDotsArray) {
            circle.alpha = 0.0;
...
-(void)changePageNumber:(UIScrollView *)scrollView{
    //NSLog(@"%f",(scrollView.contentOffset.x+ (self.contentScrollView.frame.size.width/2) - self.contentScrollView.frame.size.width/2));
    //NSLog(@"%f",((scrollView.contentSize.width - scrollView.frame.size.width)));
    ...
    //CGRect frame = CGRectIntersection(self.contentScrollView.bounds, self.backgroundXAxis.frame);
    //CGFloat proportion = (frame.size.height*frame.size.width)/(self.backgroundXAxis.frame.size.width*self.backgroundXAxis.frame.size.height);
    //NSLog(@"%f; %@", proportion, NSStringFromCGRect(frame));
    
    if (visibleRect.origin.x > 0) {
        int pageNumber = 0;
        pageNumber = round((scrollView.contentOffset.x) / _graphSpacing);
        float space = ((self.backgroundXAxis.frame.size.width) / (numberOfPoints - 1));
        if (visibleRect.size.width >= (self.contentScrollView.frame.size.width)) {
                pageNumber = (visibleRect.size.width - ((self.contentScrollView.frame.size.width) - self.YAxisLabelXOffset)) / space;
                //NSLog(@"Offset %f,Visible Rect %f",scrollView.contentOffset.x,visibleRect.size.width);
        }
        else if (visibleRect.size.width < _contentScrollView.frame.size.width/2 || (visibleRect.size.width <= scrollView.contentSize.width)){
            //NSLog(@"Initial");
            pageNumber = round((visibleRect.size.width - _contentScrollView.frame.size.width/2) / space);
        }
        _pageControl.currentPage = pageNumber;
        if ((pageNumber < circleDotsArray.count)) {
            for (BEMCircle *circle in circleDotsArray) {
        ...
        if (pageNumber < circleDotsArray.count) {
            BEMCircle *circle = [circleDotsArray objectAtIndex:pageNumber];
            circle.alpha = 1.0;
            if ([self.delegate respondsToSelector:@selector(lineGraph:didScrollClosestIndex:)]) {
                [self.delegate lineGraph:self didScrollClosestIndex:pageNumber];
            }
        }
    }
    /*if ((scrollView.contentOffset.x+self.contentScrollView.frame.size.width/2 - self.contentScrollView.frame.size.width/2) >= 0 && (scrollView.contentOffset.x + self.contentScrollView.frame.size.width/2) < (scrollView.contentSize.width - scrollView.frame.size.width)) {
        int pageNumber = 0;
        pageNumber = round((scrollView.contentOffset.x) / _graphSpacing);

        if (scrollView.contentOffset.x >= self.contentScrollView.frame.size.width) {
            pageNumber = round((scrollView.contentOffset.x - (self.contentScrollView.frame.size.width - self.YAxisLabelXOffset)/2) / _graphSpacing);
            //NSLog(@"Moving Page %d",pageNumber);
        }
        else if (scrollView.contentOffset.x > _graphSpacing){
            //NSLog(@"Initial");
            pageNumber = round((scrollView.contentOffset.x) / _graphSpacing);
        }
            _pageControl.currentPage = pageNumber;
...
- (BEMCircle *)closestDotFromtouchInputLine:(UIView *)touchInputLine {
    ...
    for (BEMCircle *point in self.contentScrollView.subviews) {
