//
//  NewRegisterationViewController.swift
//  FarmerConnect
//
//  Created by Admin on 09/07/19.
...
@objc public class CropsData : NSObject{
    ...
    public init(dict : NSDictionary) {
        ...
        self.image = (Validations.checkKeyNotAvail(dict, key: "image") as? String)!
        self.cropId = Validations.checkKeyNotAvail(dict, key: "id") as? String ?? "0"

        if let ids = dict.value(forKey: "id") as? Int {
            self.cropId = String(format: "%d",ids) as String
        }
    }
}
...
class cropsTableViewCell : UITableViewCell{
    ...
    @IBOutlet weak var btnRemoveRow: UIButton!
    @IBOutlet weak var txtCropValue: UITextField!
    @IBOutlet weak var txtCropName: UITextField!
}

class NewRegisterationViewController: BaseViewController,UITextFieldDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UITableViewDelegate,UITableViewDataSource {
    ...
    @IBOutlet weak var lblTypeofFarmer: UILabel!
    
    @IBOutlet weak var txtTypeofFarmer: UITextField!
    @IBOutlet weak var imgTypeofFarmerDropdown: UIImageView!
    
    @IBOutlet weak var viewHeightConstraint: NSLayoutConstraint!
    var arrTypeOfFarmer : NSMutableArray = NSMutableArray()
    var tblViewTypeofFarmer : UITableView = UITableView()
    ...
    override func viewDidLoad() {
        ...
        self.setLeftPaddingToTextField(txtSelectCompaniesPatronized, 10)
        ...
        self.view.bringSubview(toFront: btnCompaniesView)
        btnCropsView.addTarget(self, action: #selector(hideOrShowCropsView(_:)), for: .touchUpInside)
        btnIrrigationView.addTarget(self, action: #selector(hideOrShowIrrigationsView(_:)), for: .touchUpInside)
        btnCompaniesView.addTarget(self, action: #selector(hideOrShowCompaniesView(_:)), for: .touchUpInside)
        btnSeasonsView.addTarget(self, action: #selector(hideOrShowSeasonsView(_:)), for: .touchUpInside)
        btnAddCrops.addTarget(self, action: #selector(addRowToTableViewToAddCrops(_:)), for: .touchUpInside)

        txtPincodeRemail.delegate = self
        self.txtTotalCropAcres.delegate = self
        
        if isFromUpdate! {
            self.renderUpdateScreen()
        }else{
            self.renderRegistrationScreen()
        }
    }
    @IBAction func optyInWhatsupCheckBoxAction(_ sender : UIButton) {
        
        if btnOptInWhatsup.imageView?.image == UIImage(named: "CheckMarkGreen"){
            btnOptInWhatsup.setImage(UIImage(named: "CheckboxEmpty"), for: .normal)
            self.optInWhatsApp = false
        }else {
            btnOptInWhatsup.setImage(UIImage(named: "CheckMarkGreen"), for: .normal)
            self.optInWhatsApp = true
        }
      
    }
    @objc func removeRowFromCropsCell(_ sender : UIButton) {
        if sender.tag < arrSelectedCrops.count {
           
            if arrSelectedCrops.object(at: sender.tag) as? String == "Corn"{
                cornValue = ""
            }else if arrSelectedCrops.object(at: sender.tag) as? String == "Rice"{
                riceValue = ""
            }else if arrSelectedCrops.object(at: sender.tag) as? String == "Millet"{
                milletValue = ""
            }else if arrSelectedCrops.object(at: sender.tag) as? String == "Mustard"{
                musterdValue = ""
            }else if arrSelectedCrops.object(at: sender.tag) as? String == "Soyabean"{
                soyabeanValue = ""
            }else if arrSelectedCrops.object(at: sender.tag) as? String == "Cotton"{
                cottonValue = ""
            }else if arrSelectedCrops.object(at: sender.tag) as? String == "Deligate"{
                deligateValue = ""
            }

            arrSelectedCrops.removeObject(at: sender.tag)
            self.reloadCropsData()
            
//            for i in 0..<arrSelectedCrops.count{
//                if arrCrops.contains(arrSelectedCrops.object(at: i)){
//                    arrCrops.remove(arrSelectedCrops.object(at: i))
...
//            }
    ...
    @objc func addRowToTableViewToAddCrops(_ sender : UIButton){
        if arrSelectedCrops.count > 0 {
            if arrSelectedCrops.object(at: arrSelectedCrops.count-1) as? String  == ""{
                let cropMsg = NSLocalizedString("select_crop", comment: "")
                self.view.makeToast(cropMsg)
                return
            }
        }
        self.isDataChanged = true

        if arrSelectedCrops.count < arrCrops.count {
            arrSelectedCrops.add("")
            self.reloadCropsData()
        }else{
//            self.view.makeToast("You have reached the maximum value")
        }
    }
    ...
    func changeHiddenStatusOfCrops(isHidden : Bool){
        ...
        if isHidden {
            viewCropsHeightConstraint.constant = 40
            btnCropsView.setImage(UIImage(named: "plus"), for: .normal)
         
            updateProfileHeigthConstraint.constant = viewCropsHeightConstraint.constant + viewIrrigationHeightConstraint.constant + viewCompaniesHeightConstraint.constant + viewSeasonCultivatedHeightConstraint.constant + 200
            
            viewHeightConstraint.constant = updateProfileHeigthConstraint.constant + 250
        }else{
            btnCropsView.setImage(UIImage(named: "minus"), for: .normal)
            
            tblViewCropsHeightConstraint.constant = CGFloat(arrSelectedCrops.count * 50) + 40
            ...
            viewCropsHeightConstraint.constant = collectionViewCropsHeightConstraint.constant + tblViewCropsHeightConstraint.constant + CGFloat(size)
            
            viewHeightConstraint.constant = viewCropsHeightConstraint.constant + 650
        }
        
        collectionViewCrops.isHidden = isHidden
        tblViewCrops.isHidden = isHidden
        btnAddCrops.isHidden = isHidden
        
//        tblViewCrops.reloadData()
//
...
//
    ...
    func changeHiddenStatusOfCompanies(isHidden : Bool){
        ...
        if isHidden {
            viewCompaniesHeightConstraint.constant = 40
            btnCompaniesView.setImage(UIImage(named: "plus"), for: .normal)
        }else{
            btnCompaniesView.setImage(UIImage(named: "minus"), for: .normal)
            viewCompaniesHeightConstraint.constant = collectionViewCompaniesHeightConstraint.constant + 160
        }
      
        collectionViewCompanies.isHidden = isHidden
        txtSelectCompaniesPatronized.isHidden = isHidden
        ...
        viewHeightConstraint.constant = updateProfileHeigthConstraint.constant + 250
//
//        updateProfileHeigthConstraint.constant = 707
    ...
    func changeHiddenStatusOfSeasons(isHidden : Bool){
        ...
        if isHidden{
            viewSeasonCultivatedHeightConstraint.constant = 40
            btnSeasonsView.setImage(UIImage(named: "plus"), for: .normal)
        }else{
            btnSeasonsView.setImage(UIImage(named: "minus"), for: .normal)
            viewSeasonCultivatedHeightConstraint.constant = viewSeasonCultivatedHeightConstraint.constant + 80
        }
        
        collectionViewCultivatedSeasons.isHidden = isHidden
        lblSeasonsHint.isHidden = isHidden
...
//        updateProfileHeigthConstraint.constant = 707
    ...
    func changeHiddenStatusOfIrrigations(isHidden : Bool){
        ...
        if isHidden {
            viewIrrigationHeightConstraint.constant = 40
            btnIrrigationView.setImage(UIImage(named: "plus"), for: .normal)

        }else{
            btnIrrigationView.setImage(UIImage(named: "minus"), for: .normal)

//            viewIrrigationHeightConstraint.constant = collectionViewIrrigationsHeightConstraint.constant +  80
            viewIrrigationHeightConstraint.constant = 150

        }
     
//        btnIrrigated.isHidden = isHidden
//        btnRainfed.isHidden = isHidden
//        imgIrrigated.isHidden = isHidden
//        imgRainfed.isHidden = isHidden
        lblIrrigated.isHidden = isHidden
        lblRainfed.isHidden = isHidden
        lblLine.isHidden = isHidden
        
//        collectionViewIrrigationTypes.isHidden = isHidden
...
//        updateProfileHeigthConstraint.constant = 707
    ...
    @objc func hideOrShowCropsView(_ sender : UIButton){
       var isHidden = false
        ...
        tblViewCropsHeightConstraint.constant = CGFloat(arrSelectedCrops.count * 50) + 40
        viewCropsHeightConstraint.constant = collectionViewCropsHeightConstraint.constant + tblViewCropsHeightConstraint.constant + CGFloat(size) + 30
        
        viewHeightConstraint.constant = viewCropsHeightConstraint.constant + 650
        

//        viewCropsHeightConstraint.constant = collectionViewCropsHeightConstraint.constant + CGFloat(arrSelectedCrops.count * 52) + tblViewCropsHeightConstraint.constant + 180 + CGFloat(size)
//        viewHeightConstraint.constant = viewCropsHeightConstraint.constant + 650
...
//        if arrIrrigatedTypesSelected.count > 0 {
            irrigationsSelected = arrIrrigatedTypesSelected.componentsJoined(by: ",") as NSString
//            if arrIrrigatedSubTypesSelected.count == 0 {
//                self.view.makeToast("Select Atleast One Irrigation Type")
//                return false
//            }else {
...
//            }
    ...
    @objc func submitBtn_Touch_Up_Inside() {
        ...
        if !arrIrrigatedTypesSelected.contains("Irrigated"){
            arrIrrigatedTypesSelected.add("Irrigated")
        }
        if !arrIrrigatedTypesSelected.contains("Rainfed"){
            arrIrrigatedTypesSelected.add("Rainfed")
        }

//        else if Validations.isValidEmailAddress(txtPincodeRmobileNumber.text ?? "") == false && Validations.isNullString(txtPincodeRmobileNumber.text! as NSString) == false{
...
//        }
        ...
        if isFromUpdate ?? false {
          let isValid = checkRemianingTextFieldsValidationsAndGetValuesFromTableView()
        ...
        if net?.isReachable == true{
            if isFromUpdate == true{
                let parameters = ["mobileNumber":userObj.mobileNumber! as String, "countryId":self.countryIdFromLoginVCStr,"firstName": txtFirstName.text!,"lastName": txtLastName.text!,"pincode": txtPincodeRemail.text!,"stateName": stateName,"districtName": districtName,"region": region,"emailId": txtPincodeRemail.text ?? "","deviceId": deviceID,"deviceToken": "","totalCropAcres": txtTotalCropAcres.text ?? "" ,"villageLocation":villageLocation,"geolocation":geoLocation,"cornCropAcres":cornValue,"riceCropAcres":riceValue,"milletCropAcres":milletValue,"mustardCropAcres":musterdValue,"soyabeanCropAcres": soyabeanValue,"cottonCropAcres":cottonValue,"season":cultivatedSeasonsSelected,"irrigation":irrigationsSelected,"company":companiesSelected,"deviceType":"iOS", "subIrrigationTypes": subIrrigationsSelected ,"languageId" : "" , "languageCode" : "","referrer":userData.deepLinkingString ] as NSDictionary
    ...
    func requestToUpdateUserProfile(param : [String:String]){
        let userObj = Constatnts.getUserObject()
        SwiftLoader.show(animated: true)
...
//    }
    ...
    func renderRegistrationScreen(){
        
        self.getTypeofFarmerDataFromServer()
        lblTypeofFarmer.isHidden = false
        txtTypeofFarmer.isHidden = false
        imgTypeofFarmerDropdown.isHidden = false
        tblViewTypeofFarmer.register(UITableViewCell.self, forCellReuseIdentifier: "CellFarmer")
        
        self.submitBtn.setTitle(NSLocalizedString("submit", comment: ""), for: .normal)
        self.lblTitle?.text = NSLocalizedString("registration", comment: "")

        updateProfileView.isHidden = true
        optInView.isHidden = false
        lblEmailRpincode.text = NSLocalizedString("pincode", comment: "")
        lblMobileRvillage.text = NSLocalizedString("village_location", comment: "")
        lblTypeofFarmer.text = NSLocalizedString("typeOfFarmer", comment: "")
        ...
        txtPincodeRemail.keyboardType = .numberPad
            ...
            self.geocoder.reverseGeocodeCoordinate(self.selectLocation!) { response, error in
                ...
                if let address = response?.firstResult() {
                    DispatchQueue.main.async {
                        ...
                        self.txtPincodeRemail.text = address.postalCode
                        self.txtMobileRvillage.text = lines?.joined(separator: ",")
                        
                        self.sendPincodeToServer(str: address.postalCode ?? "000000")
...
//        self.disableMandatoryFieldsForUpdateProfile(userTypeTxtFld, isEnable: false)
        ...
        if userObj.arrSubIrrigations?.count ?? 0 > 0 {
            self.arrIrrigatedSubTypesSelected.removeAllObjects()
            self.arrIrrigatedSubTypesSelected.addObjects(from: userObj.arrSubIrrigations as! [Any])
           
            if arrIrrigatedSubTypesSelected.contains("Canal"){
                imgCanal.borderColor = App_Theme_Blue_Color
            }
            if arrIrrigatedSubTypesSelected.contains("Well"){
                imgWell.borderColor = App_Theme_Blue_Color
            }
            if arrIrrigatedSubTypesSelected.contains("Rainfed"){
                imgRainy.borderColor = App_Theme_Blue_Color
            }
        }

//        if userObj.arrIrrigations?.count ?? 0 > 0 {
//            self.arrSelectedIrrigations.removeAllObjects()
//            self.arrSelectedIrrigations.addObjects(from: userObj.arrIrrigations as! [Any])
//
//            if arrSelectedIrrigations.contains("Irrigated"){
////                imgIrrigated.image = UIImage(named: "CheckMarkGreen")
//                arrIrrigatedTypesSelected.add("Irrigated")
//            }
//
//            if arrSelectedIrrigations.contains("Rainfed"){
////                imgRainfed.image = UIImage(named: "CheckMarkGreen")
//                arrIrrigatedTypesSelected.add("Rainfed")
//            }
//        }
        
        self.reloadIrrigationsData()

        if userObj.arrCompanies?.count ?? 0 > 0 {
            self.arrSelectedCompanies.removeAllObjects()
            self.arrSelectedCompanies.addObjects(from: userObj.arrCompanies as! [Any])
        }
        if userObj.arrSeasons?.count ?? 0 > 0 {
            self.arrSelectedCultivatedSeasons.removeAllObjects()
            self.arrSelectedCultivatedSeasons.addObjects(from: userObj.arrSeasons as! [Any])
        }
        
        if userObj.arrSelectedCropsList?.count ?? 0 > 0 {
            print(userObj.arrSelectedCropsList)
            self.arrSelectedCrops.addObjects(from: userObj.arrSelectedCropsList as! [Any])
...
//        if imgIrrigated.image == UIImage(named: "CheckMarkGreen") {
//
//            arrIrrigations.add("Canal")
//            arrIrrigations.add("Well")
//
//        }else{
//            arrIrrigations.remove("Canal")
//            arrIrrigations.remove("Well")
//
//        }
//        if imgRainfed.image == UIImage(named: "CheckMarkGreen") {
//
//            arrIrrigations.add("Cloudy")
//
//        }else{
//            arrIrrigations.remove("Cloudy")
//
//        }
    }
    ...
    func getTypeofFarmerDataFromServer(){
        SwiftLoader.show(animated: true)
        ...
        let urlString:String = String(format: "%@%@", arguments: [BASE_URL,GET_TYPE_FARMER])
        Alamofire.request(urlString, method: .post, parameters: ["data" : ""], encoding: URLEncoding.default, headers: headers).responseJSON { (response) in
            SwiftLoader.hide()
            if response.result.error == nil {
                if let json = response.result.value {
                    ...
                    if responseStatusCode == STATUS_CODE_200{
                        let respData = (json as! NSDictionary).value(forKey: "response") as! NSString
                        let decryptData  = Constatnts.decryptResult(StrJson: respData as String)
                        print("Response after decrypting data type of farmer:\(decryptData)")
                        let farmerListArray = decryptData.value(forKey: "farmerTypeList") as! NSArray
                        print("farmerListArray iss",farmerListArray)
                       // self.arrTypeOfFarmer = farmerListArray as! NSMutableArray
                       //
                        self.arrCrops.removeAllObjects()
                        for i in 0 ..< farmerListArray.count{
                            if let farmerDict = farmerListArray.object(at: i) as? NSDictionary{
                                let cellData = CropsData(dict: farmerDict )
                                    self.arrTypeOfFarmer.add(cellData)
                            }
                        }
                        print("arrTypeOfFarmer iss",self.arrTypeOfFarmer)
//                        self.collectionViewCrops.reloadData()
                    }
                }
            }
    ...
    func renderUpdateScreen(){
        self.getCropsMasterDataFromServer()
        ...
        lblTypeofFarmer.isHidden = true
        txtTypeofFarmer.isHidden = true
        imgTypeofFarmerDropdown.isHidden = true
        let appdelegate = UIApplication.shared.delegate as! AppDelegate
      
        updateProfileView.isHidden = false
         optInView.isHidden = true
        lblEmailRpincode.text = NSLocalizedString("email_optional", comment: "")
        lblMobileRvillage.text = NSLocalizedString("mobile_no", comment: "")

        tblViewSelectCrops.register(UITableViewCell.self, forCellReuseIdentifier: "Cell")

        arrIrrigations.removeAllObjects()
        
        arrCultivatedSeasons.removeAllObjects()
        
//        arrCultivatedSeasons.add(NSLocalizedString("summer", comment: ""))
//        arrCultivatedSeasons.add(NSLocalizedString("rainy", comment: ""))
//        arrCultivatedSeasons.add(NSLocalizedString("post_rainy", comment: ""))
//        arrCultivatedSeasons.add(NSLocalizedString("spring", comment: ""))
        arrCultivatedSeasons.add("SUMMER")
        arrCultivatedSeasons.add("RAINY")
        arrCultivatedSeasons.add("POST-RAINY")
        arrCultivatedSeasons.add("SPRING")

        if arrCrops.count > 6 {
            collectionViewCropsHeightConstraint.constant = 100
            viewCropsHeightConstraint.constant = collectionViewCropsHeightConstraint.constant + 165
            
            self.view.updateConstraints()
        }
        self.submitBtn.setTitle(NSLocalizedString("update", comment: ""), for: .normal)
        self.lblTitle?.text = NSLocalizedString("update_profile", comment: "")
      ...
      //  self.checkCollectionViewCompaniesSize()
        ...
        if !appdelegate.isCropsUpdated{
            self.changeHiddenStatusOfCrops(isHidden: false)
            self.changeHiddenStatusOfIrrigations(isHidden: true)
            self.changeHiddenStatusOfSeasons(isHidden: true)
            self.changeHiddenStatusOfCompanies(isHidden: true)
        }else if !appdelegate.isIrrigationsUpdated {
            self.changeHiddenStatusOfCrops(isHidden: true)
            self.changeHiddenStatusOfIrrigations(isHidden: false)
            self.changeHiddenStatusOfSeasons(isHidden: true)
            self.changeHiddenStatusOfCompanies(isHidden: true)
        }else if !appdelegate.isSeasonsUpdated {
            self.changeHiddenStatusOfCrops(isHidden: true)
            self.changeHiddenStatusOfIrrigations(isHidden: true)
            self.changeHiddenStatusOfSeasons(isHidden: false)
            self.changeHiddenStatusOfCompanies(isHidden: true)
        }
        else if !appdelegate.isCompaniesUpdated {
            self.changeHiddenStatusOfCrops(isHidden: true)
            self.changeHiddenStatusOfIrrigations(isHidden: true)
            self.changeHiddenStatusOfSeasons(isHidden: true)
            self.changeHiddenStatusOfCompanies(isHidden: false)
        }else{
            self.changeHiddenStatusOfCrops(isHidden: false)
            self.changeHiddenStatusOfIrrigations(isHidden: true)
            self.changeHiddenStatusOfSeasons(isHidden: true)
      ...
      print(arrCrops)
        collectionViewCrops.reloadData()
//        collectionViewIrrigationTypes.reloadData()
        collectionViewCultivatedSeasons.reloadData()
        
//        btnIrrigated.addTarget(self, action: #selector(btnIrrigatedClicked), for: .touchUpInside)
//        btnRainfed.addTarget(self, action: #selector(btnRainfedClicked), for: .touchUpInside)
        
        btnCanal.addTarget(self, action: #selector(btnCanalClicked), for: .touchUpInside)
        btnWell.addTarget(self, action: #selector(btnWellClicked), for: .touchUpInside)
        btnCloudy.addTarget(self, action: #selector(btnCloudyClicked), for: .touchUpInside)

//
        let downBtn = UIButton(type: .custom)
        downBtn.frame = CGRect(x: 0, y: 0, width: 15, height: 15)
        downBtn.setImage(UIImage(named:"DropDown-1"), for:.normal)
        txtSelectCompaniesPatronized?.rightView = downBtn
        txtSelectCompaniesPatronized?.rightViewMode = .always
        txtSelectCompaniesPatronized?.contentMode = .center
        
        let singleTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(NewRegisterationViewController.contentViewTapGestureRecognizer(_:)))
        singleTapGestureRecognizer.numberOfTapsRequired = 1
        singleTapGestureRecognizer.isEnabled = true
        singleTapGestureRecognizer.cancelsTouchesInView = false
        updateProfileView?.addGestureRecognizer(singleTapGestureRecognizer)
    ...
    @objc func btnCanalClicked(){
        ...
        if arrIrrigatedSubTypesSelected.contains("Canal") {
            arrIrrigatedSubTypesSelected.remove("Canal")
            
            imgCanal.borderColor = UIColor.gray
            
            if arrIrrigatedTypesSelected.contains("Irrigated"){
                arrIrrigatedTypesSelected.remove("Irrigated")
            }
            
        }else{
            arrIrrigatedSubTypesSelected.add("Canal")
    ...
    @objc func btnWellClicked(){
        if arrIrrigatedSubTypesSelected.contains("Well") {
            arrIrrigatedSubTypesSelected.remove("Well")
            
            imgWell.borderColor = UIColor.gray
            
            if arrIrrigatedTypesSelected.contains("Irrigated"){
                arrIrrigatedTypesSelected.remove("Irrigated")
            }
        }else{
            arrIrrigatedSubTypesSelected.add("Well")
    ...
    @objc func btnCloudyClicked(){
        if arrIrrigatedSubTypesSelected.contains("Rainfed") {
            arrIrrigatedSubTypesSelected.remove("Rainfed")
            
            imgRainy.borderColor = UIColor.gray
            if arrIrrigatedTypesSelected.contains("Rainfed"){
                arrIrrigatedTypesSelected.remove("Rainfed")
            }

        }else{
            arrIrrigatedSubTypesSelected.add("Rainfed")
    ...
    @objc func contentViewTapGestureRecognizer(_ tapGesture:UITapGestureRecognizer) {
        ...
        tblViewTypeofFarmer.isHidden = true
    }
            ...
            if response.result.error == nil {
                if let json = response.result.value {
                    ...
                    if responseStatusCode == STATUS_CODE_200{
                        ...
                        self.loadCompaniesDataToDropDown()
...
//                        self.updateTableViewHeightAndScrollViewHeight()
    ...
    func loadCompaniesDataToDropDown(){
        self.loadDropDownTableView(tableViewDataSource: self, tableViewDelegate: self, tableview: tblViewCompanies, textField: txtSelectCompaniesPatronized!)
        tblViewCompanies.register(UITableViewCell.self, forCellReuseIdentifier: "Cell")
...
//        if imgIrrigated.image == UIImage(named: "CheckMarkGreen") {
//            imgIrrigated.image = UIImage(named: "")
//            arrIrrigations.remove("Canal")
//            arrIrrigations.remove("Well")
//            arrIrrigatedTypesSelected.remove("Irrigated")
//        }else{
...
//        if imgRainfed.image == UIImage(named: "CheckMarkGreen") {
//            imgRainfed.image = UIImage(named: "")
//            arrIrrigations.remove("Cloudy")
//            arrIrrigatedTypesSelected.remove("Rainfed")
//
//        }else{
...
//            }
        ...
        else if textField == txtTypeofFarmer{
            if(clickedFarmer == false){
                tblViewTypeofFarmer = UITableView()
                
                self.loadDropDownTableView(tableViewDataSource: self, tableViewDelegate: self, tableview: tblViewTypeofFarmer, textField: textField)
                
                tblViewTypeofFarmer.delegate = self
                tblViewTypeofFarmer.dataSource = self
                //            tblViewTypeofFarmer.reloadData()
                
                self.hideUnhideDropDownTblView(tblView: tblViewTypeofFarmer, hideUnhide: false)
                clickedFarmer = true
                tblViewTypeofFarmer.reloadData()
                return false
            }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        else if textField == tblViewTypeofFarmer{
...
//        else{
            //irrigationsCell
//            let cell = self.collectionViewIrrigationTypes.dequeueReusableCell(withReuseIdentifier: "irrigationsCell", for: indexPath)
//
//            let dashboardImg = cell.contentView.viewWithTag(101) as! UIImageView
//
//            dashboardImg.image = UIImage(named:arrIrrigations.object(at: indexPath.row) as! String)
//
//            let titleLbl = cell.contentView.viewWithTag(102) as! UILabel
//            titleLbl.text = arrIrrigations.object(at: indexPath.row) as? String
//
//            if arrIrrigatedSubTypesSelected.contains(arrIrrigations.object(at: indexPath.row)){
//                dashboardImg.borderColor = App_Theme_Blue_Color
//                dashboardImg.cornerRadius = 4.0
//                dashboardImg.borderWidth = 0.5
//            }else{
//                dashboardImg.borderColor = UIColor(red: 230.0/255.0, green: 238.0/255.0, blue: 252.0/255.0, alpha: 1.0)
//                dashboardImg.cornerRadius = 4.0
//                dashboardImg.borderWidth = 0.5
...
//        else if collectionView == collectionViewIrrigationTypes{
//            if arrIrrigatedSubTypesSelected.contains(arrIrrigations.object(at: indexPath.row)) {
//                arrIrrigatedSubTypesSelected.remove(arrIrrigations.object(at: indexPath.row))
//            }else{
//                arrIrrigatedSubTypesSelected.add(arrIrrigations.object(at: indexPath.row))
//            }
//
//            self.collectionViewIrrigationTypes.reloadData()
//        }
        else if collectionView == collectionViewCultivatedSeasons{
            if arrSelectedCultivatedSeasons.contains(arrCultivatedSeasons.object(at: indexPath.row)) {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        else if tableView == tblViewTypeofFarmer{
            return arrTypeOfFarmer.count
        }
        else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tblViewCompanies{
            ...
            let cellIdentifier = "CellFarmer"
            let cell = UITableViewCell(style: UITableViewCellStyle.default, reuseIdentifier: cellIdentifier)
            cell.textLabel?.font = UIFont.systemFont(ofSize: 13.0)
            
            let cellData = arrTypeOfFarmer.object(at: indexPath.row) as? CropsData
            cell.textLabel?.text = cellData?.name
...
//            cell.textLabel?.text = arrCrops.object(at: indexPath.row) as? String
            ...
            txtTypeofFarmer.text = cellData?.name

           // self.tblViewTypeofFarmer.isHidden = true
            self.hideUnhideDropDownTblView(tblView: tblViewTypeofFarmer, hideUnhide: true)
            clickedFarmer = false
            tblViewTypeofFarmer.reloadData()
    ...
    func checkCollectionViewCompaniesSize(){
        ...
        if arrSelectedCompanies.count > 3{
            collectionViewCompaniesHeightConstraint.constant = 120
            viewCompaniesCollectionHeightConstraint.constant = collectionViewCompaniesHeightConstraint.constant + 30
            viewCompaniesHeightConstraint.constant = viewCompaniesCollectionHeightConstraint.constant * 2
            viewHeightConstraint.constant = viewCropsHeightConstraint.constant + collectionViewCompaniesHeightConstraint.constant + 650
        }
        else{
            collectionViewCompaniesHeightConstraint.constant = 60
            viewCompaniesCollectionHeightConstraint.constant = collectionViewCompaniesHeightConstraint.constant + 30
            viewCompaniesHeightConstraint.constant = viewCompaniesCollectionHeightConstraint.constant * 2 + 10
            viewHeightConstraint.constant = viewCropsHeightConstraint.constant + collectionViewCompaniesHeightConstraint.constant + 650
        }
    }
}
