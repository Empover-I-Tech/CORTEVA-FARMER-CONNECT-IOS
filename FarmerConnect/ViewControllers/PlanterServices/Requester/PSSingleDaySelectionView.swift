//
//  PSSingleDaySelectionView.swift
//  FarmerConnect
//
...
class PSSingleDaySelectionView: UIView, UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UITextFieldDelegate,GrowingTextViewDelegate,UITextViewDelegate {
    ...
    class func instanceFromNib() -> PSSingleDaySelectionView {
        return UINib(nibName: "PSSingleDaySelectionView", bundle: nil).instantiate(withOwner: nil, options: nil)[0] as! PSSingleDaySelectionView
    }
    ...
    func getTimeSlotsForSelectedDay(_ selectedDate: String){
        if self.selectedEquipment != nil {
            ...
            Alamofire.request(urlString, method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
                ...
                if response.result.error == nil || response.result == nil {
...
//                                self.lblDistance?.text = String(format:"%d kms",minimumHours)
                            ...
                            if let arrAvailableHours = Validations.checkKeyNotAvailForArray(decryptData, key: "availableHours") as? NSArray{
                                ...
                                if self.arrAvailableSlots.count > 0{
                                    for index in 0..<self.arrAraayFormattedTimes.count {
                                       ...
                                       // print("the array is",self.arrAvailableSlots)
    ...
    func validationgOrderDetails() -> Bool{
        ...
        if placeOrder != nil && selectedEquipment != nil{
            ...
            else if Validations.isNullString(self.getAddrs as NSString? ?? "" as NSString) == true{
            ...
            else if Validations.isNullString(self.getLat as NSString? ?? "" as NSString) == true || Validations.isNullString(self.getLng as NSString? ?? "" as NSString) == true{
                placeOrderAlert = CustomAlert.alertInfoPopUpView(self, frame: UIScreen.main.bounds, title: ALERT as NSString, message: Request_Location_Not_Empty as NSString , buttonTitle: OK, hideClose: true) as? UIView
                self.appdelegate?.window?.addSubview(placeOrderAlert!)
                return false
            }
    ...
    func updateDataToUiComponents(){
            ...
            if self.selectedLocation == nil{
                if LocationService.sharedInstance.currentLocation?.coordinate != nil{
                    ...
                    if tempCurrentLocation != nil {
                        ...
                        getLat = placeOrder?.latitude as String?
                        getLng = placeOrder?.longitude as String?
        ...
        self.heightConstraint.constant = (self.timeSelectionView?.frame.maxY)! + 70
        self.layoutIfNeeded()
        self.updateConstraintsIfNeeded()
        self.scrollView?.contentSize = CGSize(width: (self.scrollView?.frame.size.width)!, height: (timeSelectionView?.frame.maxY)! + 80)
...
func generateAvaialableTimeSlotsFromServerData(arrAvailableHours: NSArray){
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        bgButton?.addTarget(self, action: #selector(PSSingleDaySelectionView.timeSlotButtonClick(_:)), for: .touchUpInside)
        ...
        geocoder.reverseGeocodeCoordinate(coordinate) { response, error in
            if let address = response?.firstResult() {
                ...
                if self.placeOrder != nil{
                    ...
                    self.getAddrs = self.placeOrder?.locationName as String?
                }
            }
        }
    }
        ...
        if delegate != nil && self.selectedEquipment != nil{
            ...
            selectLocationController?.addressCompletionBlock = {(selectedlocation ,address,postalCode,isFromAddress,fromHomeNav) in
                if Validations.isNullString(address as NSString) == false{
                    ...
                    self.layoutIfNeeded()
                    self.updateConstraintsIfNeeded()
                    self.scrollView?.contentSize = CGSize(width: (self.scrollView?.frame.size.width)!, height: (self.timeSelectionView?.frame.maxY)! + 80)
