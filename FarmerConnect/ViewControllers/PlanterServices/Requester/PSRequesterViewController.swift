//
//  PSRequesterViewController.swift
//  FarmerConnect
//
...
@objc class PSPOIItem: NSObject, GMUClusterItem {
...
let PSkClusterItemCount = 10000
let PSkCameraLatitude = -33.8
let PSkCameraLongitude = 151.2

class PSRequesterViewController: RequesterBaseViewController, GMSMapViewDelegate,GMUClusterManagerDelegate,GMUClusterRendererDelegate ,CLLocationManagerDelegate{
    ...
    override func viewDidLoad() {
        ...
        btnRatings.addTarget(self, action: #selector(PSRequesterViewController.ratingsButtonClick(_:)), for: UIControlEvents.touchUpInside)
           ...
           // print("checkbox is \(isOn ? "ON" : "OFF")")
            ...
            if isOn == true{
              ...
              self.isLocationSelectedToStopUpdates = false
                ...
                self.mapTapGesturerecognizer = UILongPressGestureRecognizer(target: self, action: #selector(PSRequesterViewController.mapViewSelectAnimationTapGesture(_:)))
        ...
        mapTapGesturerecognizer = UILongPressGestureRecognizer(target: self, action: #selector(PSRequesterViewController.mapViewSelectAnimationTapGesture(_:)))
        ...
        self.recordScreenView("PSRequesterViewController", FSR_MapView)
...
//                        let clusterItem = PSPOIItem(position: coordinates!, markerEquipment: equipment)
//                        clusterManager?.add(clusterItem)
                        ...
                        markerView.imageEquipment?.kf.setImage(with : imageUrl, placeholder: UIImage(named:"image_placeholder.png"))
    ...
    override func backButtonClick(_ sender: UIButton) {
        if isFromHome == true {
            ...
            let toSelectPayVC = self.storyboard?.instantiateViewController(withIdentifier: "PSSprayServicesStagesViewController") as? PSSprayServicesStagesViewController
    ...
    override func filterButtonClick(_ sender: UIButton) {
        ...
        let toFilterVC = self.storyboard?.instantiateViewController(withIdentifier: "PSFilterDetailsViewController") as! PSFilterDetailsViewController
...
//            let nearbyEquipListController = self.storyboard?.instantiateViewController(withIdentifier: "PSNearByEquipmentsViewController") as? PSNearByEquipmentsViewController
...
//            self.navigationController?.present(navController, animated: true, completion: nil)
            ...
            nearbyEquipListController?.arrEquipments = arrEquipmentList
                        nearbyEquipListController?.cropID = cropID
                        nearbyEquipListController?.isFromBookingStages = self.isFromBookingStages
            self.navigationController?.pushViewController(nearbyEquipListController!, animated: true)
...
//        self.navigationController?.pushViewController(requesterOrdersController!, animated: true)
    ...
    func mapView(_ mapView: GMSMapView, didTap marker: GMSMarker) -> Bool {
        self.customInfoView?.removeFromSuperview()
        if let poiItem = marker.userData as? PSPOIItem {
       ...
       self.createCustomEquipmentInfoPopupView(equipment: poiItem.equipmentObj)
            ...
            selectedMarker = marker
            self.createCustomEquipmentInfoPopupView(equipment:marker.userData as! Equipment)
        ...
        if let poiItem = marker.userData as? PSPOIItem {
            if poiItem.equipmentObj != nil {
                ...
                if Validations.isNullString((poiItem.equipmentObj?.equipmentId) ?? "") == false{
                    let viewEquipmentController = self.storyboard?.instantiateViewController(withIdentifier: "PSViewEquipmentViewController") as? PSViewEquipmentViewController
    ...
    func createCustomEquipmentInfoPopupView(equipment: Equipment?){
        if equipment != nil {
            ...
            infoWindow.btnClose?.addTarget(self, action: #selector(PSRequesterViewController.closeCustomInfoWindow(_:)), for: .touchUpInside)
