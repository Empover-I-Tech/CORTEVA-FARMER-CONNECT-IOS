...
class PSBookNowViewController: RequesterBaseViewController{
    ...
    @IBOutlet var singleDayView : PSSingleDaySelectionView?
    @IBOutlet var multipleDayView : PSMultipuleDaySelection?
    ...
    override func viewDidLoad() {
        ...
        singleDayView = PSSingleDaySelectionView.instanceFromNib()
        singleDayView?.frame = CGRect(x: 0, y: 160, width: (UIScreen.main.bounds.size.width), height: UIScreen.main.bounds.size.height - 250)
        ...
        multipleDayView = PSMultipuleDaySelection.instanceFromNib()
        multipleDayView?.frame = CGRect(x: 0, y: 160, width: (UIScreen.main.bounds.size.width), height: UIScreen.main.bounds.size.height - 250)
        ...
        if paramsDic != nil{
            ...
            Alamofire.request(urlString, method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers).responseJSON { (response) in
                ...
                if response.result.error == nil {
                    if let json = response.result.value {
                       // print("Response :\(json)")
                        let statusCode = (json as! NSDictionary).value(forKey: "statusCode") as? String ?? ""
                        if(statusCode == ""){
                            self.view.makeToast("Maximum price per hour should not be less than price per hour")
                        }
                        else{
                            let responseStatusCode = (json as! NSDictionary).value(forKey: "statusCode") as! String
                            if responseStatusCode == STATUS_CODE_200{
                                if let message  = (json as! NSDictionary).value(forKey: "message") as? String{
                                    ...
                                    self.successOrderAlert = CustomAlert.alertInfoPopUpView(self, frame: UIScreen.main.bounds, title: "" as NSString, message: message as NSString, buttonTitle: NSLocalizedString("ok", comment: ""), hideClose: true) as? UIView
...
//            if self.isFromBookingStages == true {
            let feedbackController = self.storyboard?.instantiateViewController(withIdentifier: "PSSprayServicesStagesViewController") as? PSSprayServicesStagesViewController
