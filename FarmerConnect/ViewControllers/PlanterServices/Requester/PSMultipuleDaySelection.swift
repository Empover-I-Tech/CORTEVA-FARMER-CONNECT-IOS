...
class PSMultipuleDaySelection: UIView ,UITextFieldDelegate,GrowingTextViewDelegate,UITextViewDelegate {
    
    @IBOutlet var txtFromSchedule : UITextField?
    @IBOutlet var txtToSchedule : UITextField?
    @IBOutlet var txtPricePerHour : UITextField?
    @IBOutlet var txtAcres : UITextField?
    ...
    class func instanceFromNib() -> PSMultipuleDaySelection {
        return UINib(nibName: "PSMultipuleDaySelection", bundle: nil).instantiate(withOwner: nil, options: nil)[0] as! PSMultipuleDaySelection
    }
    ...
    func commonInit(){
        ...
        btnSchedule.addTarget(self, action: #selector(btnFromScheduleAction(_:)), for: .touchUpInside)
        ...
        btnToSchedule.addTarget(self, action: #selector(btnToScheduleAction(_:)), for: .touchUpInside)
    ...
    @objc func btnFromScheduleAction(_ sender:UIButton!) {
        fromDatePickerView()
    }
    @objc func btnToScheduleAction(_ sender:UIButton!) {
        toDatePickerView()
    }
    func toDatePickerView(){
        if arrAvailableDates != nil{
            if delegate != nil && self.arrAvailableDates!.count > 0{
                ...
                print("available to date",self.arrAvailableDates!)
                ...
                bookCalanderController?.dateSelectionCompletionBlock = {(selectedDateStr, selectedDate) in
                    if selectedDate != nil || selectedDateStr != nil{
                        if selectedDateStr != nil{
                            ...
                            self.placeOrder?.toDate = selectedDateStr as NSString?
                            ...
                            self.selectedToDate = selecDate as Date?
                            print("selected from formated data",self.selectedToDate)
                            self.txtToSchedule?.text = formattedDate as String?
                            self.getTimeSlotsForSelectedDay(selectedDateStr!)
                            
                            
                    if self.selectedToDate < self.selectedFromDate
                    {
                    self.makeToast("To date should be greater than or equal to from date")
                    self.txtToSchedule?.text = ""
                    } else {
                ...
                self.txtToSchedule?.resignFirstResponder()
    ...
    func fromDatePickerView(){
        if arrAvailableDates != nil{
            if delegate != nil && self.arrAvailableDates!.count > 0{
                let bookCalanderController = self.delegate?.storyboard?.instantiateViewController(withIdentifier: "BookNowCalanderViewController") as? BookNowCalanderViewController
                print("available from date",self.arrAvailableDates!)
                bookCalanderController?.arrAvailability = NSMutableArray(array: self.arrAvailableDates!)
                bookCalanderController?.dateSelectionCompletionBlock = {(selectedDateStr, selectedDate) in
                    if selectedDate != nil || selectedDateStr != nil{
                        if selectedDateStr != nil{
                            ...
                            let selecDate = FarmServicesConstants.getDateFromDateString(dateStr: selectedDateStr!)
                            let formattedDate = FarmServicesConstants.getDateStringFromDateWithShortMonth(serverDate: selecDate as Date?)
                            self.selectedFromDate = selecDate as Date?
                            print("selected from formated data",self.selectedFromDate!)
                            self.txtFromSchedule?.text = formattedDate as String?
                            self.getTimeSlotsForSelectedDay(selectedDateStr!)
                        }
                    }
                }
                self.endEditing(true)
                self.txtFromSchedule?.resignFirstResponder()
                self.delegate?.navigationController?.pushViewController(bookCalanderController!, animated: true)
            }
        }
    }
...
//            
//            //dobPicker
//            scheduleDatePicker = UIDatePicker (frame: CGRect(x: 5,y: (Height-200)/2,width: width-10,height: 500))
...
//            
//            //scheduleDatePicker?.addTarget(self, action: #selector(PSMultipuleDaySelection.datePickerValueChanged(_:)), for: .valueChanged)
...
//            closeView.addTarget(self, action: #selector(PSMultipuleDaySelection.closeDatePicerViewButtonClick(_:)), for: .touchUpInside)
...
//            btnOK.addTarget(self, action: #selector(PSMultipuleDaySelection.alertOK), for: UIControlEvents.touchUpInside)
...
//            }
//            else{
//                fromDateView?.makeToast("Please select available date.")
//            }
//        }
//        else{
//            fromDateView?.makeToast("Please select available date.")
...
//    }
    ...
    func placeOrderForMultiDayServiceCall() -> NSDictionary?{
        if self.validationgOrderDetails() == true {
            if selectedEquipment != nil{
                ...
                paramsDic.setValue("Planter Services", forKey: "moduleType")
                paramsDic.setValue(txtAcres?.text, forKey: "requestedAcres")
                paramsDic.setValue("",forKey:"assistedType")
    ...
    func validationgOrderDetails() -> Bool{
        ...
        if placeOrder != nil && selectedEquipment != nil{
            ...
            else if Validations.isNullString(self.txtAcres?.text as NSString? ?? "" as NSString) == true{
                placeOrderAlert = CustomAlert.alertInfoPopUpView(self, frame: UIScreen.main.bounds, title: NSLocalizedString("alert", comment: "") as NSString, message: NSLocalizedString("acres_hint", comment: "") as NSString, buttonTitle: NSLocalizedString("ok", comment: ""), hideClose: true) as? UIView
                self.appdelegate?.window?.addSubview(placeOrderAlert!)
                return false
            }
...
//                let dateString = selectedEquipment?.startDate
...
//                dateComponent.day = 1
//                let AddedDate = Calendar.current.date(byAdding: dateComponent, to: dateObj!)
                ...
                if let fromDate = FarmServicesConstants.getDateStringFromDateWithShortMonth(serverDate: minDate as Date) as String?{
                    ...
                    self.selectedFromDate = minDate as Date?
                    print("selected from formated data",self.selectedFromDate!)
        ...
        self.heightConstraint.constant = (self.contentView?.frame.maxY)! + 70
        self.layoutIfNeeded()
        self.updateConstraintsIfNeeded()
    ...
    func calculateDistanceBetweenLocations(){
        ...
        if let equipmentLocation = CLLocation(latitude: ((self.selectedEquipment?.equipmentLatitude)?.doubleValue)!, longitude: ((self.selectedEquipment?.equipmentLongitude)?.doubleValue)!) as CLLocation?{
            ...
            self.makeToast(NSLocalizedString("No_Available_Dates", comment: ""))
        }
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if textField == txtFromSchedule{
            activeTextField = textField
            fromDatePickerView()
            self.endEditing(true)
            textField.resignFirstResponder()
        }
        else if textField == txtToSchedule {
            activeTextField = textField
            toDatePickerView()
            self.endEditing(true)
            textField.resignFirstResponder()
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        else if textField == multipleProvidersAlert?.viewWithTag(55555){
        let maxLength = 5
        let currentString: NSString = (textField.text ?? "") as NSString
        let newString: NSString =  currentString.replacingCharacters(in: range, with: string) as NSString
        ...
        else if textField == multipleProvidersAlert?.viewWithTag(66666){
        let maxLength = 4
        let currentString: NSString = (textField.text ?? "") as NSString
        let newString: NSString =  currentString.replacingCharacters(in: range, with: string) as NSString
