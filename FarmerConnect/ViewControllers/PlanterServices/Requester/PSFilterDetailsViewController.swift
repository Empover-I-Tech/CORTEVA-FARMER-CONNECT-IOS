//
//  PSFilterDetailsViewController.swift
//  FarmerConnect
//
...
class PSFilterDetailsViewController: RequesterBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.recordScreenView("PSFilterDetailsViewController", FSR_Filter)
...
//                        appDelegate.tempClassificationMutArray = self.classificationListMutArray
    ...
    func fromDatePickerView(){
        ...
        btnOK.addTarget(self, action: #selector(PSFilterDetailsViewController.alertOK), for: UIControlEvents.touchUpInside)
...
//        }
    ...
    func sendRequestToGetEquipmentDetails(){
        let net = NetworkReachabilityManager(host: "www.google.com")
        if net?.isReachable == true{
//            let parameters = ["latitude":Double((self.selectLocation?.latitude)!), "longitude":Double((self.selectLocation?.longitude)!),"location": txtFldRequestLocation.text!,"distance": Int(serviceAreaDistanceStr) ?? 3,"price": Int(pricePerHourStr) ?? 500,"withDriver":true,"pickAndDrop":false,"fromDate":txtFldFromDate.text!,"toDate":txtFldToDate.text!,"fromTime":"","toTime":"","moduleType": "Planter Services","equipmentClassification":1,"noOfHours":Int(txtFldNumOfHours.text!) ?? 0,"applyFilter":true,"maxPrice":Int(pricePerHourStr) ?? 500,"maxDistance":Int(pricePerHourStr) ?? 500] as NSDictionary
...
extension PSFilterDetailsViewController :  UITableViewDataSource, UITableViewDelegate{
...
extension PSFilterDetailsViewController : UITextFieldDelegate{
