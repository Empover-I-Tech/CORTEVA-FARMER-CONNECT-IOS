//
//  FiledSharingViewController.swift
//  FarmerConnect
//
//  Created by EMPOVER-049 on 23/08/21.
...
class FiledSharingViewController: BaseViewController,UIScrollViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate , UICollectionViewDelegate,UICollectionViewDataSource{
...
//            shareAndPreview.backgroundColor = .gray
            ...
            self.view.makeToast("Select Sharing Statement")
        }
        
        else{
            self.view.makeToast("Please capture or select an image")
        }
    }
    ...
    func uploadingWithMultiPartFormData( _ str : String,nmae: String){
        ...
        }, usingThreshold: 60, to: String(format :"%@%@",BASE_URL,CEP_fIELDSHARING), method: .post, headers: headers, encodingCompletion: {(encodeResult) in
            switch encodeResult {
    ...
    @IBAction func facebookShareAction(_ sender: Any) {
        /*   let sharePhoto = SharePhoto()
         sharePhoto.image = img_upload.image
         sharePhoto.caption =  self.selectedStr
        ...
        /*  let shareDialog = ShareDialog()
         ...
         shareLinkContent.contentURL = NSURL(string: "https://www.google.com") as! URL
         shareLinkContent
        ...
        //
        //        var sharebtn = FBShareButton()
        //        sharebtn.frame = CGRect(x: 0, y: 0, width: 100, height: 100)
        //        let sharePhoto = SharePhoto()
        //        sharePhoto.image = img_upload.image
        //        sharePhoto.caption =  self.selectedStr
        //        let content = ShareMediaContent()
    ...
    override func viewWillAppear(_ animated: Bool) {
        facebookLbl.text = "RHRD_FACEBOOK".localized
        whatsappLbl.text = "RHRD_Whatsap".localized
        ...
        self.lblTitle?.text = NSLocalizedString("cep_field_Picture_Sharing", comment: "")
    ...
    @IBAction func whatsAppShare(_ sender: Any)
    {
        let urlWhats = "whatsapp://app"
           if let urlString = urlWhats.addingPercentEncoding(withAllowedCharacters:CharacterSet.urlQueryAllowed) {
               if let whatsappURL = URL(string: urlString) {
                   ...
                   if UIApplication.shared.canOpenURL(whatsappURL as URL) {
                       let getImage:UIImage = img_upload.image!
                      // if let image = UIImage(named: "CropsBtnBackgroundImg") {
                           if let imageData = UIImageJPEGRepresentation(getImage, 1.0) {
        ...
        */
        ...
        /*   guard let iconImage = img_upload.image else { return  }
         
         let savePath = NSHomeDirectory().appending("/Documents/whatsAppTmp.wai")
         //NSHomeDirectory().stringByAppendingString("/Documents/whatsAppTmp.wai")
         
         // UIImageJPEGRepresentation(iconImage!, 1.0)?.writeToFile(savePath, atomically: true)
         
         let urlWhats = "whatsapp://app"
         if let imageData = UIImageJPEGRepresentation(iconImage, 1.0) {
         let tempFile = URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent("Documents/whatsAppTmp.wai")
         do {
         try imageData.write(to: tempFile, options: .atomic)
         documentInteractionController = UIDocumentInteractionController.init()
         ...
         documentInteractionController.uti = "net.whatsapp.image"
         let imageURL = NSURL.fileURL(withPath: savePath)
...
extension FiledSharingViewController : UIDocumentInteractionControllerDelegate{
...
extension FiledSharingViewController: SharingDelegate {
